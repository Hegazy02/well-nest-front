...
const PatientDetails = () => {
  ...
  const contactInfo = [
    ...
    { label: 'Address', value: patient.AddressId?.street || 'N/A' },
  ...
  const medicalInfo = [
    ...
    { label: 'Allergies', value: patient.medicalInfoId?.allergies?.join(', ') || 'None' },
    { label: 'Chronic Diseases', value: patient.medicalInfoId?.chronicDiseases?.join(', ') || 'None' },
    ...
    <div className="flex items-center space-x-4">
      <div className="flex-shrink-0">
        <img
          ...
          src={patient.image || 'https://placehold.co/200x200?text=No+Image'}
          alt={patient.fullName}
        />
      </div>
      <div>
        <h2 className="text-xl font-semibold text-gray-900">{patient.fullName}</h2>
        <p className="text-sm text-gray-500">ID: {patient.serialNumber || 'N/A'}</p>
      </div>
    </div>
    <PrimaryButton
      variant="outline"
