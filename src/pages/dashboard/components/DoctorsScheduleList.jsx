...
const DoctorsScheduleList = ({ className}) => {
  ...
  return (
    <div className={`bg-white rounded-2xl p-6 border border-[#eee] ${className}`}>
      ...
      {loading ? (
        ...
        <div className="h-[200px] flex items-center justify-center text-red-400">{error}</div>
      ) : (
        <ul>
          {data.map((doc, idx) => (
            <li key={doc.doctorId} className={`flex items-center py-4 ${idx !== 0 ? 'border-t border-[#eee]' : ''}`}>
              ...
              <div className="ml-4 flex-1">
                <div className="font-semibold text-base text-gray-900">{doc.name}</div>
                <div className="text-gray-400 text-sm">{doc.department}</div>
              </div>
              <div className="flex flex-col items-end">
                <span className={`px-4 py-1 rounded-full text-sm font-medium border ${doc.todayAppointmentCount > 0 ? 'bg-cyan-50 text-cyan-700 border-cyan-200' : 'bg-white text-red-400 border-red-300'}`}>
