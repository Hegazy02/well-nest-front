...
const Card = ({ icon, label, count, diff, diffPercent, subtext }) => (
  ...
  >
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontWeight: 500, fontSize: 18 }}>{label}</span>
    </div>
    ...
    <div style={{ color: "#888", marginTop: 8, fontSize: 16 }}>{subtext}</div>
  </div>
);
  ...
  return (
    <div className="grid xl:grid-cols-4 gap-4 lg:grid-cols-3 md:grid-cols-2 xs:grid-cols-1">
      {cardMeta.map(({ key, label, icon }) => {
        const d = data[key] || {};
        const diff = d.yesterdayAppointmentsCount - d.appointmentCount;
        const diffPercent = d.differentPercentage?.toFixed(2) || 0;
        const subtext = `${Math.abs(diff)} ${
          diff >= 0 ? "less" : "more"
        } than yesterday`;
        return (
          <Card
